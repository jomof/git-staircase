use super::manipulation::step_commit_groups;
use super::rewrite::replay;
use crate::core::persistence;
use crate::error::{Result, StaircaseError};
use crate::git::GitRepo;
use crate::model::{
    DraftAttachment, DraftClassification, DraftFileSnapshot, DraftIntent, DraftSnapshot, Step,
    WorktreeDraft,
};
use std::fs;
...
pub fn materialize_draft(
    ...
    let groups = step_commit_groups(repo, &original_metadata)?;
    ...
    let restack_info = match intent {
        ...
        DraftIntent::NewStep => {
            ...
            Some((step_idx + 2, step_idx + 1))
        }
        DraftIntent::RewriteStep(_) => {
            meta.steps[step_idx].cut = commit_oid.clone();
            Some((step_idx + 1, step_idx + 1))
        }
    };

    if let Some((restack_start, old_groups_start)) = restack_info {
        if restack_start < meta.steps.len() {
            let upper_groups = groups[old_groups_start..].to_vec();
            replay(
                repo,
                &managed,
                meta,
            ...
            // Replay handles publishing metadata and finishing the operation.
            return Ok(MaterializeResult {
                staircase_id: original_metadata.id,
                staircase_name: original_metadata.name,
                step_name: original_metadata.steps[step_idx].name.clone(),
                commit_oid,
                updated_steps_count: 1 + upper_groups.len(),
            });
        }
    }
