...
fn test_staircase_storage_alignment() {
    ...
    let oid = repo.resolve_ref(ref_name).expect("refs/staircases/auth should exist");
    ...
    assert!(content.starts_with("git-staircase-descriptor 1"), "Descriptor should start with header, but was: {}", content);
    ...
    assert!(repo.resolve_ref(&meta_ref).is_err(), "Old meta ref should not exist");
    ...
    assert!(new_content.contains(&c2_new), "New descriptor should contain new cut OID");
    
    // Verify it still resolves via name
    let found = core::find_by_name(&repo, "auth").unwrap().expect("Should find auth by name");
