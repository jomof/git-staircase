...
use git_staircase::core::manipulation::{restack, RebaseOptions};
...
fn test_restack_manual_rebase_of_multiple_steps() {
    ...
    let tree_oid = repo.write_tree(&[TreeEntry::blob(blob_oid.clone(), "file")]).unwrap();
    let root_commit = repo.command()
    ...
    restack(&repo, &staircase, RebaseOptions { leave_upper_steps_stale: false }).expect("Restack should succeed after fix");
    
    // 6. Verify that the staircase is now updated and clean
    let id = staircase.metadata().id.clone();
    let latest_metadata = git_staircase::core::persistence::read_metadata(&repo, &id).expect("Metadata should be readable");
    
    assert_eq!(latest_metadata.steps[0].cut, a1, "Step 1 cut should be updated to a1");
    assert_eq!(latest_metadata.steps[1].cut, a2, "Step 2 cut should be updated to a2");
    
    let status = git_staircase::core::status::get_status_metadata(&repo, latest_metadata, true).unwrap();
