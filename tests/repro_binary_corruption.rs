...
fn test_binary_blob_corruption() {
    ...
    Command::new("git").current_dir(dir).args(&["add", "binary.bin"]).output().unwrap();
    Command::new("git").current_dir(dir).args(&["commit", "-m", "binary"]).output().unwrap();
    
    let oid_output = Command::new("git").current_dir(dir).args(&["rev-parse", "HEAD:binary.bin"]).output().unwrap();
