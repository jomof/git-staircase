...
fn test_repro_ancestry_exclusion_bug() {
    ...
    Command::new("git").arg("config").arg("user.name").arg("test").current_dir(repo_path).output().unwrap();

    // Create A -> B -> C
    Command::new("git").arg("commit").arg("--allow-empty").arg("-m").arg("A").current_dir(repo_path).output().unwrap();
    let a_oid = Command::new("git").arg("rev-parse").arg("HEAD").current_dir(repo_path).output().unwrap().stdout;
    let a_oid = String::from_utf8(a_oid).unwrap().trim().to_string();

    Command::new("git").arg("commit").arg("--allow-empty").arg("-m").arg("B").current_dir(repo_path).output().unwrap();
    let b_oid = Command::new("git").arg("rev-parse").arg("HEAD").current_dir(repo_path).output().unwrap().stdout;
    let b_oid = String::from_utf8(b_oid).unwrap().trim().to_string();

    Command::new("git").arg("commit").arg("--allow-empty").arg("-m").arg("C").current_dir(repo_path).output().unwrap();
    let c_oid = Command::new("git").arg("rev-parse").arg("HEAD").current_dir(repo_path).output().unwrap().stdout;
    ...
    repo.preload_ancestry_ext(&[&c_oid], &[&b_oid, &a_oid]).unwrap();
