...
fn journey_1_bootstraps_repo_gerrit_and_publishes_three_reviews() {
  ...
  <project name="my-project" path="." />
    ...
    assert!(stdout.contains("Step 3"), "Should discover the staircase: {}", stdout);
    
    let (ok, stdout, stderr) = ctx.run_staircase(&["adopt", "my-staircase", "--onto", "main", "step1", "step2", "step3"]);
    assert!(ok, "adopt failed: {}", stderr);
    
    // ASSERT: Verify adoption and metadata
    assert!(stdout.contains("my-staircase"), "Should report adoption of 'my-staircase': {}", stdout);
...
fn journey_1_amend_preserves_draft_and_review_identity_across_conflicts() {
    ...
    let (ok, _, stderr) = ctx.run_staircase(&["adopt", "my-staircase", "--onto", "main", "step1", "step2", "step3"]);
    assert!(ok, "adopt failed: {}", stderr);

    // ACT: Amend the bottom step (oid1)
    // We need to checkout the first branch, amend it, and then restack.
    ctx.run_git(&["checkout", "step1"]);
    ctx.commit("file1.txt", "content1 amended", "commit 1 amended\n\nChange-Id: I1111111111111111111111111111111111111111");
    ...
    for (i, expected_id) in ["I1111111111111111111111111111111111111111", 
                             "I2222222222222222222222222222222222222222", 
                             "I3333333333333333333333333333333333333333"].iter().enumerate() {
        let oid = steps[i]["cut"].as_str().unwrap();
        let message = ctx.run_git(&["log", "-1", "--format=%B", oid]);
        assert!(message.contains(expected_id), "Step {} should preserve Change-Id {}: {}", i, expected_id, message);
    }
}
...
fn journey_4_cross_worktree_materialization_preserves_partial_staging() {
    ...
    ctx.run_git(&["branch", "step1", &oid1]);
    ctx.run_git(&["branch", "step2", &oid2]);
    let (ok, _, stderr) = ctx.run_staircase(&["adopt", "my-staircase", "--onto", "main", "step1", "step2"]);
    assert!(ok, "adopt failed: {}", stderr);
    ...
    let (ok, stdout, stderr) = common::run_staircase(&wt_path, &["draft", "materialize", "my-staircase", "--fold-into", "step1", "materialized.txt"]);
    ...
    assert!(!status.contains("A  staged.txt"), "staged.txt should be committed, not staged: {}", status);
    assert!(status.contains("?? unstaged.txt"), "unstaged.txt should still be untracked: {}", status);
    ...
    assert!(files.contains("materialized.txt"), "New commit should contain materialized.txt: {}", files);
    assert!(files.contains("staged.txt"), "New commit SHOULD ALSO contain staged.txt because it was in the index: {}", files);
}
