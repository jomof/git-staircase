use git_staircase::core::{ArchiveOptions, ResolvedSelector, archive_staircase};
...
fn test_archive_staircase_leaks_worktree_detach_on_failure() {
    ...
    // Checkout main in the primary repo so feat1 is free to be used in another worktree
    std::process::Command::new("git")
        .current_dir(&repo_dir)
        .args(&["checkout", "main"])
        .output()
    ...
    let record_ref = git_staircase::core::refs::StaircaseRefs::record(
        &managed_meta.id,
    ...
    assert!(
        result.is_err(),
        "Archiving should have failed due to concurrent update"
    );
    ...
    let wt1 = worktrees
        .iter()
        .find(|w| w.path.to_string_lossy().contains("wt1"))
        .expect("Should find wt1");

    assert!(
        wt1.branch.is_some(),
        "Worktree should NOT be left detached on failure. Found state: {:?}",
        wt1
    );
    assert_eq!(wt1.branch.as_deref(), Some("refs/heads/feat1"));
}
