...
fn test_worktrees_correctly_parses_multiple_worktrees() {
    ...
    assert_eq!(worktrees.len(), 2, "Should have found 2 worktrees, found: {:?}", worktrees);
    
    // Check if both paths are present
    let paths: Vec<PathBuf> = worktrees.iter().map(|w| w.path.canonicalize().unwrap()).collect();
    let expected_root = root_path.canonicalize().unwrap();
    let expected_wt = wt_path.canonicalize().unwrap();
    
    assert!(paths.contains(&expected_root), "Root path {:?} not in {:?}", expected_root, paths);
    assert!(paths.contains(&expected_wt), "Worktree path {:?} not in {:?}", expected_wt, paths);
}
