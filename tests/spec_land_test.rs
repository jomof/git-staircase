...
fn test_land_stepwise_integration() {
    ...
    let (ok, stdout, stderr) = ctx.run_staircase(&["adopt", "auth", "--onto", "main", "--landing-policy", "stepwise", "step1", "step2"]);
    ...
    assert_eq!(main_oid, c2, "main should be at the tip of the staircase after stepwise land");
...
fn test_land_aggregate_integration() {
    ...
    let (ok, stdout, stderr) = ctx.run_staircase(&["adopt", "auth", "--onto", "main", "--landing-policy", "aggregate-only", "step1", "step2"]);
    assert!(ok, "Adopt failed: {} {}", stdout, stderr);
