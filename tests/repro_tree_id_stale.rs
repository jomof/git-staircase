...
fn test_repro_tree_id_stale_bug() {
    ...
    Command::new("git").arg("add").arg("file.txt").current_dir(repo_path).output().unwrap();
    Command::new("git").arg("commit").arg("-m").arg("A").current_dir(repo_path).output().unwrap();
    let tree_a = Command::new("git").arg("rev-parse").arg("HEAD^{tree}").current_dir(repo_path).output().unwrap().stdout;
    ...
    let tree_b = Command::new("git").arg("rev-parse").arg("HEAD^{tree}").current_dir(repo_path).output().unwrap().stdout;
    ...
    assert_eq!(res_b, tree_b, "Should have gotten tree of B, but got tree of A (stale cache)!");
}
