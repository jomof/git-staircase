...
#[test]
fn test_stale_memoization_repro() {
    ...
    // ARRANGE: Resolve branch and cache it
    let resolved1 = repo.resolve_commit("main").unwrap();
    assert_eq!(resolved1, oid1);

    // ACT: Update main externally and call fetch (which should clear memoizer but doesn't)
    ...
    // Simulate fetch (which currently doesn't clear memoizer)
    repo.fetch("origin", &[], true).unwrap_or(()); // "origin" doesn't exist but command runs

    // ASSERT: Resolve main again, it should be the NEW OID if memoizer was cleared
    let resolved2 = repo.resolve_commit("main").unwrap();
    assert_eq!(resolved2, oid2, "Memoized OID is stale after fetch!");
}
