...
fn test_duplicate_step_names_leak() {
    ...
    let metadata = StaircaseMetadata {
        ...
        steps: vec![
            Step { id: "s1".to_string(), name: "duplicate".to_string(), cut: c1.clone(), branch: None },
            Step { id: "s2".to_string(), name: "duplicate".to_string(), cut: c2.clone(), branch: None },
        ],
    ...
    assert!(!refs.is_empty(), "Commit c1 should have at least one ref pointing to it for reachability");
}
