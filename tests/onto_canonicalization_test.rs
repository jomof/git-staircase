...
fn test_adopt_canonicalizes_onto() {
    ...
    let (success, _stdout, stderr) = ctx.run_staircase(&["adopt", "my-staircase", "--onto", "main", "feature"]);
    ...
    assert!(content.contains("target-ref refs/heads/main"), "Descriptor should contain full refname for target-ref, but was:\n{}", content);
    assert!(!content.contains("target-ref main\n"), "Descriptor should NOT contain abbreviated target-ref name");
}
...
fn test_infer_onto_canonicalizes() {
    ...
    assert!(content.contains("target-ref refs/heads/main"), "Descriptor should contain full refname for inferred target-ref, but was:\n{}", content);
}
