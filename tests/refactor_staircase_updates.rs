...
fn setup_repo() -> (TempDir, GitRepo) {
    ...
    Command::new("git").current_dir(&repo_path).args(["add", "f"]).status().unwrap();
    Command::new("git").current_dir(&repo_path).args(["commit", "-m", "initial"]).status().unwrap();

    // Step A
    Command::new("git").current_dir(&repo_path).args(["checkout", "-b", "branch-a"]).status().unwrap();
    fs::write(repo_path.join("f"), "A1\n").unwrap();
    Command::new("git").current_dir(&repo_path).args(["add", "f"]).status().unwrap();
    Command::new("git").current_dir(&repo_path).args(["commit", "-m", "commit A1"]).status().unwrap();
    fs::write(repo_path.join("f"), "A2\n").unwrap();
    Command::new("git").current_dir(&repo_path).args(["add", "f"]).status().unwrap();
    Command::new("git").current_dir(&repo_path).args(["commit", "-m", "commit A2"]).status().unwrap();

    // Step B
    Command::new("git").current_dir(&repo_path).args(["checkout", "-b", "branch-b"]).status().unwrap();
    fs::write(repo_path.join("f"), "B\n").unwrap();
    Command::new("git").current_dir(&repo_path).args(["add", "f"]).status().unwrap();
    Command::new("git").current_dir(&repo_path).args(["commit", "-m", "commit B"]).status().unwrap();
...
fn test_managed_staircase_updates() {
    ...
    let metadata = StaircaseMetadata {
        ...
        steps: vec![
            Step { name: "A".to_string(), cut: a2_oid.clone(), branch: Some("branch-a".to_string()) },
            Step { name: "B".to_string(), cut: b_oid.clone(), branch: Some("branch-b".to_string()) },
        ],
    ...
    let rs = core::resolve_staircase(&repo, "test-managed", None).unwrap().unwrap();
    assert_eq!(rs.metadata().steps.len(), 3);
    assert_eq!(rs.metadata().steps[0].name, "A-part1");
    assert!(repo.resolve_ref("refs/staircases/test-managed/steps/A-part1").is_ok());

    // Join Step A-part1 and A
    core::join(&repo, &rs, 0, 1).unwrap();
    let rs = core::resolve_staircase(&repo, "test-managed", None).unwrap().unwrap();
    assert_eq!(rs.metadata().steps.len(), 2);
    assert!(repo.resolve_ref("refs/staircases/test-managed/steps/A-part1").is_err());
