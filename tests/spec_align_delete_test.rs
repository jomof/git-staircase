...
fn test_delete_implicit_staircase() {
    ...
    assert!(!success, "Delete command should fail for implicit staircase without flag: {}", stdout);
    assert!(stderr.contains("use --delete-materializing-refs"), "Error message should suggest flag: {}", stderr);
    
    // Verify branches are still there
    assert!(!context.run_git(&["rev-parse", "--verify", "feature-1"]).is_empty());
    assert!(!context.run_git(&["rev-parse", "--verify", "feature-2"]).is_empty());

    // 2. Now delete with --delete-materializing-refs -> Should succeed
    let (success, _stdout, stderr) = context.run_staircase(&["delete", "feature-2", "--delete-materializing-refs"]);
...
fn test_delete_managed_staircase() {
    ...
    assert!(success);
    assert!(stdout.contains("\"is_implicit\": false"), "Managed staircase should have is_implicit: false in JSON");
    ...
    // Verify branches are still there
    assert!(!context.run_git(&["rev-parse", "--verify", "managed-1"]).is_empty());
    assert!(!context.run_git(&["rev-parse", "--verify", "managed-tip"]).is_empty());
    
    // Verify managed records are gone (nothing should have is_implicit: false)
    let (success, stdout, _) = context.run_staircase(&["list", "--json"]);
    assert!(success);
    assert!(!stdout.contains("\"is_implicit\": false"), "No staircase should be managed anymore");

    // Re-adopt
    let (success, _, _) = context.run_staircase(&["adopt", "managed-again", "managed-1", "managed-tip"]);
    assert!(success);

    // Delete managed staircase WITH deleting owned branches
    let (success, _stdout, stderr) = context.run_staircase(&["delete", "managed-again", "--delete-owned-branches"]);
